...
namespace Pim\ProductBundle\Services\AttributeSet\Show\Version1\Processor;

use AppBundle\Rest\Api\Manager as ApiManager;
use Pim\ProductBundle\Database\Manager as DbM;
use Pim\ProductBundle\Services\AttributeSet\Show\Version1\Request\Request as RequestModel;
use Pim\ProductBundle\Services\AttributeSet\Show\Version1\Response\Response as ResponseModel;
use Pim\ProductBundle\Services\AttributeSet\Show\Version1\Response\AttributeSet as SingleAttributeSetResponse;
use Pim\ProductBundle\Entity\AttributeSet as AttributeSetEntity;
use Pim\ProductBundle\Services\Common\Error;

/**
 * Class Index
 * @package Pim\ProductBundle\Services\AttributeSet\Show\Version1\Processor
 */
class Index
{
    /**
    ...
    public function run(RequestModel $requestModel): ResponseModel
    {
        try {
            ...
            if(null === $attributeSetEntity){
                ...
                $this->responseModel = $this->mapToResponse($attributeSetEntity);
    ...
    /**
     * @param AttributeSetEntity $attributeSets
     * @return ResponseModel
     */
    private function mapToResponse(AttributeSetEntity $attributeSets)
    {
        return $this->apiM->mapper()->modelInToModelOut($attributeSets, $this->responseModel);
    }
